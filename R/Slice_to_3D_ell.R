...
#' @param MCMCparams named list containing details about the running of MCMC, the components are niters (integer, number of posterior samples to be drawn), nthin(integer, the thinning interval), nburn (integer, number of posterior samples to be discarded as burn-in samples), adapt (Boolean, whether an adaptive scheme should be used to make the acceptance ratio close to 0.574, defaults to TRUE). Total number of iterations = niters*nthin
#'
#' @useDynLib DimProj
#'
#' @import Rcpp
...
Slice_to_3D_ell <- function(data,K,
  ...
  ll <- Slicesto3D_ell(Y,S,bma,as.integer(gridsize[1]),as.integer(gridsize[2]),
                   mu_init,rho_init,A_init,sig2_init,
                   MCMCparams$niters,MCMCparams$nburn,MCMCparams$nthin,as.integer(MCMCparams$adapt),
